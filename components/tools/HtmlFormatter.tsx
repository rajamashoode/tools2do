...
export function HtmlFormatter(): React.ReactElement {
  const [input,setInput]   = useState('');
  const [output,setOutput] = useState('');
  function format() {
    let depth = 0;
    const tags = input.replace(/></g,'>\n<').split('\n');
    const formatted = tags.map(tag => {
      if (/^<\//.test(tag.trim())) depth = Math.max(0, depth - 1);
      const line = '  '.repeat(depth) + tag.trim();
      if (/^<[^/!]/.test(tag.trim()) && !/<.*\/>/.test(tag) && !/<(br|hr|img|input|link|meta)/i.test(tag)) depth++;
      return line;
    });
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 md:px-6 space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <textarea value={input} onChange={e=>setInput(e.target.value)} placeholder="Paste your HTML here..." rows={18} className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-4 font-mono text-sm outline-none resize-y focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]"/>
