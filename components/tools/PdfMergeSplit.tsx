...
declare global {
  interface Window { PDFLib: PDFLib; }
}

function parsePageRanges(s: string, max: number): number[] {
  const set: number[] = [];
  s.split(',').forEach(part => {
    const [a, b] = part.trim().split('-').map(x => parseInt(x.trim()));
    if (!isNaN(a)) {
      const end = isNaN(b) ? a : b;
      for (let i = a; i <= end && i <= max; i++) if (i > 0) set.push(i - 1);
    }
  });
...
export function PdfMergeSplit(): React.ReactElement {
  ...
  function loadFiles(fl: FileList | File[]): void {
    ...
    if (!valid.length) { setError('Please select PDF files.'); return; }
  ...
  async function split(): Promise<void> {
    if (!files.length) { setError('Select a PDF to split.'); return; }
    if (!pages.trim()) { setError('Enter page numbers or ranges, e.g. 1,3,5-7'); return; }
    setBusy(true); setError(''); setPdfUrl('');
    try {
      setStatus('Loading PDF library...');
      await loadPDFLib();
      ...
      if (!indices.length) { setError('No valid pages found. Check your input.'); setBusy(false); return; }
      const out = await PDFDocument.create();
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-10 md:px-6 space-y-5">
      ...
      >
        ...
        <p className="mt-3 font-bold text-[var(--text-primary)]">Drag and drop PDF files, or click to upload</p>
        <p className="mt-1 text-sm text-[var(--text-secondary)]">Merge multiple PDFs · or split pages from one PDF · runs in your browser</p>
        <input ref={inputRef} type="file" accept="application/pdf" multiple onChange={(e: ChangeEvent<HTMLInputElement>) => { if (e.target.files?.length) loadFiles(e.target.files); }} className="hidden" />
      </div>
      ...
      {files.length > 0 && (
        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 space-y-4">
          <div className="space-y-2">
            {files.map((f, i) => (
              <div key={`${f.name}-${i}`} className="flex items-center gap-3 rounded-xl border border-[var(--border-default)] bg-[var(--bg-canvas)] p-3">
                ...
                <button onClick={() => removeFile(i)} className="text-xs text-[var(--danger)] rounded-lg border border-[var(--border-default)] px-2 py-1 hover:bg-[var(--bg-surface-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]">✕</button>
              </div>
            ))}
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="block text-sm font-semibold mb-1 text-[var(--text-primary)]">Pages to extract (for Split)</label>
              <input value={pages} onChange={e => setPages(e.target.value)} placeholder="e.g. 1,3,5-7" className="w-full rounded-xl border border-[var(--border-default)] bg-[var(--bg-canvas)] px-3 py-2.5 text-sm outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]" />
            </div>
            <div>
              <label className="block text-sm font-semibold mb-1 text-[var(--text-primary)]">Output file name</label>
              <div className="flex gap-2 items-center">
                <input value={outName} onChange={e => setOutName(e.target.value)} placeholder="tools2do-pdf" className="flex-1 rounded-xl border border-[var(--border-default)] bg-[var(--bg-canvas)] px-3 py-2.5 text-sm outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]" />
          ...
          <div className="flex gap-3 flex-wrap">
            <button onClick={merge} disabled={busy} className="rounded-xl bg-[var(--accent-action)] px-5 py-3 font-bold text-white disabled:opacity-60 hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]">
              {busy ? status : 'Merge PDFs'}
            </button>
            <button onClick={split} disabled={busy} className="rounded-xl border border-[var(--border-default)] px-5 py-3 font-semibold text-sm disabled:opacity-60 hover:bg-[var(--bg-surface-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]">
              Split First PDF
            </button>
      ...
      {pdfUrl && (
        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 text-center space-y-3">
          ...
          <a href={pdfUrl} download={`${outName || 'tools2do-pdf'}.pdf`} className="inline-flex items-center gap-2 rounded-xl bg-[var(--accent-action)] px-6 py-3 font-bold text-white hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]">
            <FileDown className="h-5 w-5" />Download {outName || 'tools2do-pdf'}.pdf
          </a>
        </div>
      )}
    </section>
