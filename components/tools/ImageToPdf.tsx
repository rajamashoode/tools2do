...
export function ImageToPdf(): React.ReactElement {
  ...
  async function convert(): Promise<void> {
    ...
    try {
      ...
      const { jsPDF } = (window as unknown as {
      ...
      setStatus(
        `Creating PDF from ${files.length} image${files.length > 1 ? 's' : ''}...`
      );
      ...
      setStatus(
        `Done! PDF ready — ${files.length} page${files.length > 1 ? 's' : ''}.`
      );
    } catch (e) {
    ...
    <section className="mx-auto max-w-3xl px-4 py-10 md:px-6 space-y-5">
      ...
      >
        ...
        <p className="mt-1 text-sm text-[var(--text-secondary)]">
          Each image becomes one PDF page · order is adjustable · runs in your browser
        </p>
      ...
      {files.length > 0 && (
        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 space-y-4">
          <p className="font-semibold text-[var(--text-primary)]">
            Images ({files.length}) — drag to reorder
          </p>
          ...
          <div className="space-y-2">
            {files.map((f, i) => (
              ...
              >
                <span className="text-xs font-bold text-[var(--text-muted)] w-5 shrink-0">
                  {i + 1}
                </span>
                ...
                <div className="flex gap-1 shrink-0">
                  <button
                    ...
                    onClick={() => moveUp(i)}
                    disabled={i === 0}
                    className="rounded-lg border border-[var(--border-default)] px-2 py-1 text-xs disabled:opacity-30 hover:bg-[var(--bg-surface-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]"
                  >
                    ↑
                  </button>
                  ...
                  <button
                    ...
                    className="rounded-lg border border-[var(--border-default)] px-2 py-1 text-xs text-[var(--danger)] hover:bg-[var(--bg-surface-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]"
                  >
                    ✕
                  </button>
                </div>
      ...
      {pdfUrl && (
        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 text-center space-y-3">
          ...
          <a
            href={pdfUrl}
            download={`${pdfName || 'images-to-pdf'}.pdf`}
