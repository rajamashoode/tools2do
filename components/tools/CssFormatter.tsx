...
export function CssFormatter(): React.ReactElement {
  const [input, setInput]   = useState('');
  const [output, setOutput] = useState('');
  function format() {
    setOutput(input.replace(/\s*}\s*/g,'\n}\n').replace(/\s*{\s*/g,' {\n  ').replace(/;\s*/g,';\n  ').replace(/\n\s*\n/g,'\n').trim());
  }
  function minify() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 md:px-6 space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <textarea value={input} onChange={e=>setInput(e.target.value)} placeholder="Paste your CSS here..." rows={18} className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-4 font-mono text-sm outline-none resize-y focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]"/>
