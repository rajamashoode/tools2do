...
function parseTargetSize(size?: string): number | null {
  if (!size) return null;

  const cleaned = size.trim().toLowerCase();
  const match = cleaned.match(/^(\d+(?:\.\d+)?)\s*(kb|mb|b)?$/);
...
export function ImageCompressor({
  targetSize,
}: ImageCompressorProps): React.ReactElement {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const blob = targetBytes
          ? await compressToTarget(file, targetBytes)
          : await compressFile(file, quality / 100);
  ...
  async function compressToTarget(file: File, target: number): Promise<Blob> {
    let bestBlob = await compressFile(file, 0.8);

    const qualities = [0.75, 0.65, 0.55, 0.45, 0.35, 0.25, 0.18, 0.12];

    for (const q of qualities) {
      const blob = await compressFile(file, q);
  ...
  function updateName(i: number, val: string): void {
    setResults((prev) =>
      prev.map((r, idx) => (idx === i ? { ...r, customName: val } : r))
    );
  }
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-10 md:px-6 space-y-5">
      ...
      >
        <ImageUp
          className="mx-auto h-10 w-10 text-[var(--accent-action)]"
          aria-hidden="true"
        />
        ...
        <p className="mt-1 text-sm text-[var(--text-secondary)]">
          JPEG, PNG, WebP · multiple files supported · runs in your browser
          ...
          {!targetSize && (
            <div>
              ...
              <input
                ...
                className="w-full accent-[var(--accent-action)]"
              />
            </div>
          )}

          {targetSize && (
            <p className="rounded-xl bg-[var(--bg-canvas)] px-4 py-3 text-sm text-[var(--text-secondary)]">
              This page will try to reduce each image close to {targetSize}.
            </p>
          )}
      ...
      {results.length > 0 && (
        <div className="space-y-4">
          <p className="font-bold text-[var(--text-primary)]">
            Results — {results.filter((r) => r.compressedUrl).length} compressed
          </p>
              ...
              {r.compressedUrl ? (
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    ...
                    <p className="text-sm text-[var(--text-secondary)]">
                      {formatBytes(r.originalSize)} · {r.width}×{r.height}
                    </p>
                  </div>

                  <div>
                    <p className="text-xs font-semibold uppercase text-[var(--text-muted)] mb-2">
                      Compressed — saved {savings(r.originalSize, r.compressedSize)}%
                    </p>

                    <img
                      src={r.compressedUrl}
                      alt="Preview"
                      className="h-24 rounded-xl object-contain bg-[var(--bg-canvas)] w-full mb-2"
                    />

                    <p className="text-sm text-[var(--text-secondary)] mb-3">
                      {formatBytes(r.compressedSize)}
                      {targetBytes && r.compressedSize > targetBytes
                        ? ` · above ${targetSize}`
                        : ''}
                    </p>
                    ...
                    <div className="flex gap-2 items-center flex-wrap">
                      ...
                      <a
                        ...
                        download={`${r.customName || 'compressed'}.${ext(r.type)}`}
                        className="inline-flex items-center gap-1.5 rounded-xl bg-[var(--accent-action)] px-4 py-2 text-sm font-bold text-white hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-indigo)]"
                      >
                ...
                <p className="text-sm text-[var(--danger)]">
                  Could not compress: {r.originalName}
                </p>
              )}
            </div>
          ))}
